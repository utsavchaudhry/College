...
int process_request(const char *host, uint16_t port, char *resource){
    ...
    //---------------------------------------------------------------------------------
    // Implement Send/Receive loop for Connection:Closed
    //---------------------------------------------------------------------------------
    ...
    // 2. Send the request to the HTTP server
    ...
    // 3. Loop and receive the response data from the server
    while (1) {
        int bytes_recvd = recv(sock, recv_buff, BUFF_SZ, 0);
        if (bytes_recvd > 0) {
            // 4. Print out the data received
