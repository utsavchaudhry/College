...
arp_packet_t *process_arp(raw_packet_t raw_packet) {
    ...
    arp->arp_hdr.htype = ntohs(arp->arp_hdr.htype);  // Hardware type
    arp->arp_hdr.ptype = ntohs(arp->arp_hdr.ptype);  // Protocol type
    arp->arp_hdr.op = ntohs(arp->arp_hdr.op);    // Operation code
...
void print_arp(arp_packet_t *arp){
    ...
    printf("     hlen:      %u\n", arp->arp_hdr.hlen);
    printf("     plen:      %u\n", arp->arp_hdr.plen);
    printf("     op:        %u (ARP ", arp->arp_hdr.op);
...
icmp_echo_packet_t *process_icmp_echo(icmp_packet_t *icmp){
    ...
    echo->icmp_echo_hdr.timestamp = ntohs(echo->icmp_echo_hdr.timestamp);
    echo->icmp_echo_hdr.timestamp_ms = ntohs(echo->icmp_echo_hdr.timestamp_ms);
...
void print_icmp_echo(icmp_echo_packet_t *icmp_packet){
    ...
    printf("     payload:   %lu bytes\n", ICMP_Payload_Size(icmp_packet));
    
    struct tm *tm_info = localtime(&(icmp_packet->icmp_echo_hdr.timestamp));

    char time_buffer[30]; 
    strftime(time_buffer, sizeof(time_buffer), "%Y-%m-%d %H:%M:%S", tm_info);
    
    printf("     ECHO Timestamp: TS = %s.%05u\n", time_buffer, icmp_packet->icmp_echo_hdr.timestamp_ms);
