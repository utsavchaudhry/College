...
class Circle2D {
  ...
  constructor() {
    ...
    document.getElementById("slide").addEventListener("input", function (e) {
      const radius = parseFloat(e.target.value);

      Circle2D.vertexPositions = getCircleCoordinates(radius, 360, 0.4, 0.35);

      gl.bindBuffer(gl.ARRAY_BUFFER, Circle2D.positionBuffer);
      gl.bufferData(
        gl.ARRAY_BUFFER,
        flatten(Circle2D.vertexPositions),
        gl.DYNAMIC_DRAW
      );
