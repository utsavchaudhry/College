...
function configureTexture(gl, texture, imageOrNull) {
  gl.bindTexture(gl.TEXTURE_2D, texture);

  if (imageOrNull) {
    gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA,
                  gl.UNSIGNED_BYTE, imageOrNull);

    if (isPowerOf2(imageOrNull.width) && isPowerOf2(imageOrNull.height)) {
    ...
    // 1×1 blue placeholder so the sampler is always renderable
    const pixel = new Uint8Array([0, 0, 150, 255]);
...
(function() {
  ...
  // --- Texture setup (unit 0) ---
  const texture = gl.createTexture();
  gl.activeTexture(gl.TEXTURE0);
  configureTexture(gl, texture, null);  // bind placeholder immediately
  gl.uniform1i(gl.getUniformLocation(program, "uTexture"), 0);

  const image = new Image();
  // If you host the image on another origin, uncomment:
  // image.crossOrigin = "anonymous";
  image.onload = function () {
    gl.activeTexture(gl.TEXTURE0);
    configureTexture(gl, texture, image); // replace placeholder with real image
    renderOnce = true;                     // redraw when ready
  };
  // Use any detailed image (faces, etc.) that you include with the submission:
  ...
  // Shading / Material
  let shadingMode = 1; // 0=gouraud, 1=phong
  ...
  function updateBuffers() {
    ...
    // normals  (bind before bufferData + pointer)
    gl.bindBuffer(gl.ARRAY_BUFFER, normalBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(currentModel.normals), gl.STATIC_DRAW);
    gl.vertexAttribPointer(aNormalLoc, 3, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(aNormalLoc);

    // texcoords (bind before bufferData + pointer)
    gl.bindBuffer(gl.ARRAY_BUFFER, texCoordBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(currentModel.texCoords), gl.STATIC_DRAW);
  ...
  function applyMaterial(i) {
    ...
    gl.uniform3fv(uDiffuseProdLoc, new Float32Array([1, 1, 1]));
